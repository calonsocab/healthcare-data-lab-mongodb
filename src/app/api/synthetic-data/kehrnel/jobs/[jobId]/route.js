...
export async function GET(req, props) {
  ...
  try {
    ...
    const result = await service.getSyntheticJob(binding.envKey, params.jobId, {
    ...
    console.error(`GET /api/synthetic-data/kehrnel/jobs/${params?.jobId} error:`, error);
    const status = error.status && Number.isInteger(error.status) ? error.status : 500;
    return NextResponse.json(
      { code: error.code || null, error: error.message || 'Failed to fetch synthetic job', details: error.details || null },
      { status }
    );
  }
}
