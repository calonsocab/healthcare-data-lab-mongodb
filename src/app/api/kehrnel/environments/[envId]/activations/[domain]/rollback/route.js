...
// src/app/api/kehrnel/environments/[envId]/activations/[domain]/rollback/route.js
...
export async function POST(req, props) {
  ...
  try {
    ...
    const envGate = await enforceEnvironmentControl(db, envId, ENV_CONTROL_CAPABILITY.OPS, {
      route: 'api/kehrnel/environments/activations/rollback'
    });
    if (envGate) return envGate;

    const service = createKehrnelService(db);
    const result = await service.rollbackActivation(envId, domain, { reason, requestId });
    ...
    try {
      ...
      console.warn('audit log failed (rollback)', err.message);
    }

    return NextResponse.json(result);
  } catch (error) {
    return safeUpstreamError(error, 'Rollback failed');
  }
}
